...
串由 **<span style="color:red;">零个或多个字符</span>** 组成的有限序列。
由一个或多个空格组成的串称为 **<span style="color:red;">空格串</span>**

### 模式匹配算法

> 自串的定位运算通常称为串的 **<span style="color:red;">模式匹配</span>** 或者 **<span style="color:red;">串匹配</span>** 。

#### BF 算法

> BF 算法思路直观简明。但当匹配失败时，主串的指针 j 总是回溯到`i-j+2`位置，模式串的指针总是恢复到首字符位置 j=1，因此算法**时间复杂度高**, 是<span style="color:red;">O(n*m)</span>
...
> 该算法是 BF 算法的改进版，可以在<span style="color:red;">O(n+m)</span>的时间数量级上完成串的模式匹配。其改进在于：每当一趟匹配过程中出现字符比较不等时，**不需回溯 i 指针**，而是利用已经得到的"部分匹配"的结果将模式向右“滑动”尽可能远的一段距离后，继续进行比较。
...
**下标从 1 开始 <span style="color:red;">next[1] = 0</span>** (A)
